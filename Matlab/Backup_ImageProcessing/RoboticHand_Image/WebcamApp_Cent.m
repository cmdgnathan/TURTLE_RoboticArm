...
% USER PARAMETERS
    ...
    img.roi.x_n = 12; % Number of Horizontal Histogram Boxes
    img.roi.y_n = 12; % Number of Vertical Histogram Boxes
    ...
    tic    
        ...
        for i = pad:img.roi.x_n-1-pad
            for j = pad:img.roi.y_n-1-pad
                img.hist(img.hist_i).roi.rect = [ round( img.x_cent - (img.crop.w/2) + (i/img.roi.x_n)*img.crop.w + 1 ) ...
                                                  round( img.y_cent - (img.crop.h/2) +  (j/img.roi.y_n)*img.crop.h + 1 ) ...
...
%         img.hist_n = img.hist_i - 1;
    ...
    tic
        for i=1:img.hist_n
            ...
            hist_crop = imcrop(hand, img.hist(img.hist_i).roi.rect);
...
%         end
    ...
    tic
        ...
        imshow(palm_blob); hold on;
        for i=1:img.hist_n
            rectangle('Position',img.hist(i).roi.rect, ...
