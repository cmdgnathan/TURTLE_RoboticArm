...
%             hand_mask = hand_mask_bw;
        ...
        % DETERMINE PEAKS AND MINS OF (IMG.INTERESECT)
        %         counter=0;
        %         testdata=zeros(100,1);
        %         if length(img.intersect)==100
        %             counter=counter+1; 
        %             testdata(:,counter)=img.intersect
        %         end
...
%         %LEFT HAND: CURRENT POSITION
            ...
            %?????????? NEED TO DO THE BINS
%             thumb=max(img.intersect());
%             index=max(img.intersect());
%             middle=max(img.intersect());
%             ring=max(img.intersect());
%             pinky=max(img.intersect());
